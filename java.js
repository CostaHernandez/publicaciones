ToggleSideMenuBtnDat = {
  ...
  function PenfilterToggle(index) {
    var li = Array.from(document.querySelectorAll("#PenFilter>li"));
    for (let i of li) {
      i.style.color = "#505061";
    }
    li[index].style.color = "white";
    var PenFilterContHr = document.querySelector("#PenFilter>hr");
    PenFilterContHr.style.left = li[index].offsetLeft + "px";
    PenFilterContHr.style.width = li[index].offsetWidth + "px";
    PenFilterContHr.setAttribute("index", index);
  }
  ...
  MainBody.onwheel = (e) => {
    if (MainBody.clientHeight + MainBody.scrollTop >= MainBody.scrollHeight) {
      ...
      if (e.deltaY < 0 && scrolLef < 0) {
        ...
        for (let i of document.querySelectorAll("#PenContianer>.Pen")) {
  ...
  function ToggleSideMenu(btn) {
    ...
    if (SideNav.offsetWidth == 0) {
      ...
      if (window.innerWidth > 801) {
        MainBody.style.width = "calc(100% - 200px)";
      } else {
      ...
      document.querySelector("#logo").style.left = "-200px";
    ...
    setTimeout(function () {
      PenfilterToggle(
        parseInt(document.querySelector("#PenFilter>hr").getAttribute("index"))
      );
    }, 300);
  }
  PenfilterToggle(1);
  ...
  // For Mobile Screen We will make a slider which works with swipe
  function GoTopFunc() {
    MainBody.scrollTo({
      top: 0,
      behavior: "smooth"
